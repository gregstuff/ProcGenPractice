...
public class GridDecorationMatcherSO : DecorationMatcherSO
{
    ...
    [Header("Coverage Pass (optional, e.g., torches)")]
    [SerializeField] private bool _enableCoveragePass = false;
    [SerializeField] private GameObject _coveragePrefab;          // e.g., Torch
    [SerializeField] private int _coverageSpacing = 4;            // every Nth candidate
    [Tooltip("Cells eligible to host coverage objects (e.g., Floor).")]
    [SerializeField] private TileMatchingRuleSO _coveragePlacementRule;
    [Tooltip("Neighbor cell rule that must be adjacent (e.g., Wall).")]
    [SerializeField] private TileMatchingRuleSO _coverageSupportRule;
    [Tooltip("Use 4-neighborhood for support detection; off = 8-neighborhood.")]
    [SerializeField] private bool _coverageUse4Neighbors = true;

    // Per-run caches
    private Dictionary<Vector2Int, List<DecorationRule>> _dimensionsToDecorationRules; // (w,h) -> rules
    private Dictionary<DecorationRule, int> _ruleIndex; // stable per ruleset order
    ...
    public override IEnumerable<IDecorationMatch> GetDecorationMatches(ICapabilityProvider level)
    {
        ...
        var candidates = GenerateCandidates(tiles);     // Phase 1
        var results = ResolveCandidates(candidates, tiles); // Phase 2

        // Optional coverage pass (post)
        if (_enableCoveragePass && _coveragePrefab != null && _coveragePlacementRule != null)
        {
            var extended = new List<IDecorationMatch>(results);
            var hardBlocked = BuildOccupiedSetFromResults(extended); // block where we already placed
    ...
    private struct Candidate
    {
        ...
        public Vector2Int Anchor;          // spawn cell
        public List<Vector2Int> HardCells; // anchor + post blocks + irregular footprint
        public int Priority;               // Rule.Priority
        public int Tie;                    // deterministic tie-breaker
    }
                ...
                for (int h = 1; h <= _maxHeight && y + h <= rows; ++h)
                    for (int w = 1; w <= _maxWidth && x + w <= cols; ++w)
                    {
                        ...
                        foreach (var rule in rules)
                        {
                            ...
                            int tie = StableHash(_randomSeed, anchor.x, anchor.y, rIndex);
    ...
    private static List<Vector2Int> BuildHardBlockCells(int originX, int originY, DecorationRule rule, Vector2Int anchor)
    {
        ...
        // Post-spawn blocks are defined relative to the pattern origin
        ...
        // 🔹 Irregular footprint: offsets relative to the anchor
    ...
    // ---------- Phase 2: Resolve (priority + spacing) ----------
    ...
    private IEnumerable<IDecorationMatch> ResolveCandidates(List<Candidate> candidates, TileTypeSO[,] tiles)
    {
        ...
        // Sort: Priority DESC, then deterministic tie
        candidates.Sort((a, b) => {
        ...
        // Hard blocks + spacing reservations
        var hardBlocked = new HashSet<Vector2Int>();

        // Reserve radii per category/prefab as sets of cells where **anchors** cannot be placed
        ...
        var personalKeepout = new HashSet<Vector2Int>(); // generic keep-out radius (for all anchors)
        ...
        foreach (var c in candidates)
        {
            ...
            // 2) Spacing checks (fast because we reserve radii sets on accept)
            if (!string.IsNullOrEmpty(c.Rule.CategoryId) && c.Rule.SameCategoryMinChebyshev > 0)
            {
                if (IsBlocked(blockedByCategory, c.Rule.CategoryId, c.Anchor)) continue;
            }
            if (c.Rule.SamePrefabMinChebyshev > 0)
            {
                if (IsBlocked(blockedByPrefab, c.Rule.Prefab, c.Anchor)) continue;
            }
            if (c.Rule.BlockRadiusChebyshev > 0)
            {
                if (personalKeepout.Contains(c.Anchor)) continue;
            }

            // 3) Budget & chance (successful placements)
            if (!_spaceMap.TryGetValue(c.Anchor, out var spaceId)) continue;
            if (!budgets.TryConsume(c.Rule, spaceId, rng)) continue;

            // 4) Accept: commit footprint & spacing reservations
            for (int i = 0; i < c.HardCells.Count; ++i)
                hardBlocked.Add(c.HardCells[i]);

            if (!string.IsNullOrEmpty(c.Rule.CategoryId) && c.Rule.SameCategoryMinChebyshev > 0)
                AddRadiusBlock(blockedByCategory, c.Rule.CategoryId, c.Anchor, c.Rule.SameCategoryMinChebyshev, rows, cols);

            if (c.Rule.SamePrefabMinChebyshev > 0)
                AddRadiusBlock(blockedByPrefab, c.Rule.Prefab, c.Anchor, c.Rule.SamePrefabMinChebyshev, rows, cols);

            if (c.Rule.BlockRadiusChebyshev > 0)
                AddRadiusBlock(personalKeepout, c.Anchor, c.Rule.BlockRadiusChebyshev, rows, cols);
    ...
    private IEnumerable<IDecorationMatch> GenerateCoverageMatches(TileTypeSO[,] tiles, HashSet<Vector2Int> occupied)
    {
        ...
        for (int y = 0; y < rows; ++y)
        {
            ...
            {
                var t = tiles[y, x];
                if (_coveragePlacementRule != null && !_coveragePlacementRule.MatchesTile(t)) continue;
        ...
        for (int i = 0; i < candidates.Count; i += Mathf.Max(1, _coverageSpacing))
        {
            var p = candidates[i];
    ...
    {
        if (_coverageSupportRule == null) return true; // optional
        int rows = tiles.GetLength(0), cols = tiles.GetLength(1);

        // Neighbor deltas
        if (_coverageUse4Neighbors)
        {
            // Up, Down, Left, Right
            int[] dx = { 0, 0, -1, 1 };
            int[] dy = { -1, 1, 0, 0 };
            for (int k = 0; k < 4; ++k)
            {
                ...
                if (nx >= 0 && nx < cols && ny >= 0 && ny < rows)
                    if (_coverageSupportRule.MatchesTile(tiles[ny, nx])) return true;
            }
        }
        else
        {
    ...
    private sealed class RuleBudgetContext
    {
        ...
        public bool TryConsume(DecorationRule rule, string spaceId, System.Random rng)
        {
